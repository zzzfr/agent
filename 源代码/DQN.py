...
def rand_sample_mask(action_space,mask_list):
...
class DQN:
    ...
    def __init__(self, env, learning_rate=0.002, gamma=0.95, buffer_size=1000000, T=10):
        self.env = env
    ...
    def take_action(self, state):
        state = torch.FloatTensor(np.array([state])).to(self.device)
        q_values = self.model(state).detach().cpu().numpy()[0]
        mask_list = list(np.where(self.env.node_road[self.env.current] == 0)[0]) + self.env.path
