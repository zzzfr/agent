...
from data import get_data, get_model_name,judge_arrive,find_max_power_path
import os
import random
...
def DQN_test(env, num, model_path, max_steps=500):
    ...
    return last_power,path_list,last_time

# 生成指定
def generate_specific_ev(end,max_power,consumption):
    [start] = random.sample([i for i in range(100) if i != end], 1)
    init_power = random.randint(10, min(max_power,50))
...
if __name__ == "__main__":
    ...
    count = 0
    while count < 10:
        ev = generate_specific_ev(0,44,10)
        if judge_arrive(ev, env_info):
            ev_list.append(ev)
            count += 1
    for EV in ev_list:
        if find_max_power_path(EV, env_info):
            path = find_max_power_path(EV, env_info)
    ...
    for i in range(len(ev_list)):
        env = EVs_Env(ev_list[i], env_info)
        env.reset()
        # # 加载保存的模型权重
        model_path = f'./model/model_0_40_10.pth'
        print(DQN_test(env, i,model_path))
        print(path_list[i],ev_list[i]['init_power'])
        print()
